...
void * objets_Sprite[2] = {(void*) icone_null_Sprite, (void*) icone_epee1_Sprite};
...
Sprite::Sprite(int type, int id, Taille t) {
    this->id = id;
    this->type = type;
    taille.setLargeur(t.getLargeur());
    taille.setHauteur(t.getHauteur());
    screen = -1;
    affiche = false;
    nSprite = -1;
}
...
int Sprite::Sprite_double_touche(int sprite1, int sprite2){
    int sprite_touche=-2;
    int sprite_deja_touche=-1;
    while(1){
        sprite_deja_touche=sprite_touche;
        sprite_touche=Sprite_touche(sprite1,sprite2);
        
        if(sprite_deja_touche==sprite_touche)
            return sprite_touche;
    }
}

int Sprite::Sprite_touche(int sprite1, int sprite2){
    int i=sprite1;
    while(1){
        PA_WaitFor(Stylus.Newpress);
        for (i = sprite1; i < sprite2 +1; i++) {
